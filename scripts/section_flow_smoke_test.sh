...
BUILD_DIR="${1:-"$ROOT_DIR/build-section-flow-smoke"}"
WORK_DIR="$(mktemp -d)"
...
if ! grep -q "waiting for central sender on port 7000" \
    "$WORK_DIR/proxy.log"; then
    echo "proxy did not start listening in time" >&2
    cat "$WORK_DIR/proxy.log" >&2
    exit 1
fi
...
grep -q "central confirmed section=0" "$WORK_DIR/central.log"
grep -q "central confirmed section=3" "$WORK_DIR/central.log"
grep -q "central confirmed cached transfer_id=" "$WORK_DIR/central.log"
grep -q "central section cached section=0" "$WORK_DIR/proxy.log"
grep -q "sending section=3" "$WORK_DIR/proxy.log"
grep -q "MISSING missing_blocks=" "$WORK_DIR/proxy.log"
