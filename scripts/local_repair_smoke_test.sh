#!/usr/bin/env bash
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
BUILD_DIR="${1:-"$ROOT_DIR/build-local-repair-smoke"}"
WORK_DIR="$(mktemp -d)"

SENDER_PID=""
RECEIVER1_PID=""
RECEIVER2_PID=""

cleanup() {
    for pid in "$SENDER_PID" "$RECEIVER1_PID" "$RECEIVER2_PID"; do
        if [[ -n "$pid" ]] && kill -0 "$pid" 2>/dev/null; then
            kill "$pid" 2>/dev/null || true
        fi
    done
    rm -rf "$WORK_DIR"
}
trap cleanup EXIT

cmake -S "$ROOT_DIR" -B "$BUILD_DIR" -DCMAKE_BUILD_TYPE=Debug
cmake --build "$BUILD_DIR" -j"$(nproc)"

dd if=/dev/urandom of="$WORK_DIR/input.bin" bs=1024 count=256 status=none
mkdir -p "$WORK_DIR/receiver-1" "$WORK_DIR/receiver-2"

"$BUILD_DIR/site_proxy_sender" \
    239.255.42.99 5000 127.0.0.1 \
    6000 2 100 0 3 \
    "$WORK_DIR/input.bin" \
    >"$WORK_DIR/sender.log" 2>&1 &
SENDER_PID="$!"

for _ in {1..50}; do
    if grep -q "waiting for 2 receiver control connections" \
        "$WORK_DIR/sender.log"; then
        break
    fi
    if ! kill -0 "$SENDER_PID" 2>/dev/null; then
        echo "sender exited before accepting receivers" >&2
        cat "$WORK_DIR/sender.log" >&2
        exit 1
    fi
    sleep 0.1
done

if ! grep -q "waiting for 2 receiver control connections" \
    "$WORK_DIR/sender.log"; then
    echo "sender did not start listening in time" >&2
    cat "$WORK_DIR/sender.log" >&2
    exit 1
fi

SRCAST_DROP_INITIAL_BLOCKS=3 "$BUILD_DIR/receiver_agent" \
    239.255.42.99 5000 "$WORK_DIR/receiver-1" \
    127.0.0.1 6000 1 127.0.0.1 \
    >"$WORK_DIR/receiver-1.log" 2>&1 &
RECEIVER1_PID="$!"

SRCAST_DROP_INITIAL_BLOCKS=3 "$BUILD_DIR/receiver_agent" \
    239.255.42.99 5000 "$WORK_DIR/receiver-2" \
    127.0.0.1 6000 2 127.0.0.1 \
    >"$WORK_DIR/receiver-2.log" 2>&1 &
RECEIVER2_PID="$!"

wait "$SENDER_PID"
SENDER_PID=""
wait "$RECEIVER1_PID"
RECEIVER1_PID=""
wait "$RECEIVER2_PID"
RECEIVER2_PID=""

INPUT_SHA="$(sha256sum "$WORK_DIR/input.bin" | awk '{print $1}')"
OUTPUT1="$(find "$WORK_DIR/receiver-1" -maxdepth 1 -name 'transfer-*.bin' -print -quit)"
OUTPUT2="$(find "$WORK_DIR/receiver-2" -maxdepth 1 -name 'transfer-*.bin' -print -quit)"

if [[ -z "$OUTPUT1" || -z "$OUTPUT2" ]]; then
    echo "receiver output missing" >&2
    cat "$WORK_DIR/sender.log" >&2
    exit 1
fi

[[ "$(sha256sum "$OUTPUT1" | awk '{print $1}')" == "$INPUT_SHA" ]]
[[ "$(sha256sum "$OUTPUT2" | awk '{print $1}')" == "$INPUT_SHA" ]]

grep -q "MISSING missing_blocks=" "$WORK_DIR/sender.log"
grep -q "repair round=1" "$WORK_DIR/sender.log"
grep -q "test hook dropped initial DATA block_id=3" "$WORK_DIR/receiver-1.log"
grep -q "test hook dropped initial DATA block_id=3" "$WORK_DIR/receiver-2.log"

echo "local repair smoke test passed"
